...
public class Brochete : TestVanDammeAnim
{
	...
	protected override void RunGun()
	{
		if (this.gunFrame > 0)
		{
			...
			if (this.gunCounter > 0.0334f)
			{
				...
				this.gunFrame--;
				this.SetGunSprite(this.gunFrame, 0);
				if (this.gunFrame == 7 || this.gunFrame == 3)
				{
	...
	{
		this.fireCount++;
		float b = this.GetKnifeSprayAngle(false);
		this.knifeSprayAngle = Mathf.LerpAngle(this.knifeSprayAngle, b, 0.33f);
		this.knifeSprayDirection = global::Math.Point3OnCircle((720f + this.knifeSprayAngle - 13.25f + 7.5f * (float)(this.fireCount % 3) + 2.5f * (float)(this.fireCount % 6)) / 180f * 3.14159274f, 1f);
		this.ThrowMachete(this.macheteSprayProjectile, this.x + this.knifeSprayDirection.x * 10f, this.y + this.knifeSprayDirection.y * 8f + 8f, this.knifeSprayDirection.x * this.knifeSpraySpeed, this.knifeSprayDirection.y * this.knifeSpraySpeed);
